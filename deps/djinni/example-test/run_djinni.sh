...
java_out="$base_dir/generated-src/java/com/dropbox/testapp"
node_out="$base_dir/generated-src/nodejs"


java_package="com.dropbox.testapp"
...
if [ $# -eq 0 ]; then
    ...
    for dir in "$temp_out" "$cpp_out" "$jni_out" "$java_out" "$node_out"; do
        if [ -e "$dir" ]; then
            echo "Deleting \"$dir\"..."
            rm -r "$dir"
        fi
...
"$base_dir/../src/run-assume-built" \
    ...
    --objc-swift-bridging-header "Test-App-Bridging-Header" \
    \
...
mirror "node" "$temp_out/nodejs" "$node_out"
